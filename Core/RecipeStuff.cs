...
namespace GoldLeaf.Core
{
    public static class RecipeCallbacks
    {
        ...
        {
            if (player.GetModPlayer<GoldLeafPlayer>().craftTimer <= 0)
            {
        ...
        public static void StarCraftEffect(Recipe recipe, Item item, List<Item> consumedItems, Item destinationStack)
        {
            if (player.GetModPlayer<GoldLeafPlayer>().craftTimer <= 0)
            {
                player.GetModPlayer<GoldLeafPlayer>().craftTimer = 25;
            ...
            {
                DustHelper.DrawStar(player.Center, DustID.FireworkFountain_Yellow, 5, 1.8f, 0.8f, 0.55f, 0.6f, 0.5f, true, 0, -1);
            }
        }
        public static void AnvilCraftEffect(Recipe recipe, Item item, List<Item> consumedItems, Item destinationStack)
        {
            if (player.GetModPlayer<GoldLeafPlayer>().craftTimer <= 0)
            {
                ...
                {
                    var d = Dust.NewDustDirect(player.Center, 12, 0, DustID.Torch, 0f, Main.rand.NextFloat(-2.8f, -4.2f), 0, default, Main.rand.NextFloat(0.7f, 0.9f));
                    //d.fadeIn = 1f;
                }
