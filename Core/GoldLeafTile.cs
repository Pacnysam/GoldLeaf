...
namespace GoldLeaf.Core
{
    internal class GoldLeafTile : GlobalTile
    {
        ...
        public override void RandomUpdate(int i, int j, int type)
        {
            ...
            if (oxeyeSafeTiles.Contains(Framing.GetTileSafely(i, j).TileType) && !TileID.Sets.Platforms[type] && Main.rand.NextBool(oxeyeChance) && !Main.tile[i, j].TopSlope && (Main.tileCut[tile.TileType] || TileID.Sets.BreakableWhenPlacing[tile.TileType] || !tile.HasTile) && !tile.IsActuated && Main.dayTime)
            {
