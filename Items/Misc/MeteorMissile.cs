...
namespace GoldLeaf.Items.Misc
{
    ...
    public class MeteorMissileP : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int j = 0; j < 10; j++)
            {
                var dust = Dust.NewDustDirect(Projectile.Center - new Vector2(16, 16), 0, 0, DustType<SpecialSmoke>());
                dust.velocity = Main.rand.NextVector2Circular(1.8f, 1.8f);
                dust.scale = Main.rand.NextFloat(0.45f, 0.8f);
                dust.alpha = 45 + Main.rand.Next(50);
    ...
    public class Ember : ModProjectile //nabbed from slr
    {
        ...
        public override void AI()
        {
            ...
            {
                var dust = Dust.NewDustPerfect(Projectile.Center, DustType<SpecialSmokeFast>(), Main.rand.NextVector2Circular(1.5f, 1.5f));
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            if (meteorMissile && target.life <= 0 && hit.Crit && cooldown <= 0)
            {
                for (int k = 0; k < 6; k++)
                {
                    ...
                    int p = Projectile.NewProjectile(Player.GetSource_OnHit(target), player.Center, pos, ProjectileType<MeteorMissileP>(), damageDone / 4, 3, player.whoAmI);
            ...
            else if (meteorMissile && target.life <= 0 && cooldown <= 0)
            {
                for (int k = 0; k < 4; k++)
                {
                    ...
                    int p = Projectile.NewProjectile(Player.GetSource_OnHit(target), player.Center, pos, ProjectileType<MeteorMissileP>(), damageDone / 3, 3, player.whoAmI);
                    Main.projectile[p].DamageType = hit.DamageType;
                    Main.projectile[p].frame = k;
                }
                cooldown = 120;
            ...
            else if (meteorMissile && hit.Crit && cooldown <= 0)
            {

                int p = Projectile.NewProjectile(Player.GetSource_OnHit(target), player.Center, new Vector2(Main.rand.NextFloat(-4f, 4f), -3f), ProjectileType<MeteorMissileP>(), damageDone / 3, 3, player.whoAmI);
                Main.projectile[p].DamageType = hit.DamageType;
